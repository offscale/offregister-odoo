...
def install0(*args, **kwargs):
    ...
    if not exists('/etc/apt/sources.list.d/odoo.list'):
        ...
        append('/etc/apt/sources.list.d/odoo.list',
...
def configure1(*args, **kwargs):
    ...
    upload_template(resource_filename('offregister_odoo', path.join('conf', 'nginx-sites-available.conf')),
                    '/etc/nginx/sites-enabled/odoo', use_sudo=True, backup=False,
