...
def install_docker0(*args, **kwargs):
    ...
    append('/etc/apt/sources.list.d/docker.list',
    ...
    if not docker_running and dist == 'precise':
        ...
        if (lambda reboot: reboot.lower() in frozenset(('y', 'yes')))(prompt('Reboot? [y|n]')):
            sudo('reboot')
...
def _destroy_docker_container(name, destroy=None):
    container_id = run('docker ps -a --format "{{.ID}}" -f' + 'name={name}'.format(name=name))
    ...
    if container_id:
        ...
        if destroy is False or destroy is None and not (lambda reboot: reboot.lower() in frozenset(('y', 'yes'))
...
def setup_postgres1(*args, **kwargs):
    ...
    run('docker run -d -e POSTGRES_USER={user} -e POSTGRES_PASSWORD={password} --name db postgres:9.4'.format(
        user=gen_random_str(randint(10, 20)), password=gen_random_str(randint(10, 20))
    ))
...
def setup_odoo2(*args, **kwargs):
    ...
    upload_template(resource_filename('offregister_odoo', path.join('conf', 'odoo.conf')),
                    '/etc/init/odoo.conf', use_sudo=True,
